...
const Footer = ({ contacts = FOOTER_CONTACTS, infos = FOOTER_INFOS }) => (
  <footer className="relative border-t border-border" id="contact">
    <div className="pointer-events-none absolute left-1/2 top-1/2 z-0 h-[80%] w-[80vw] -translate-x-1/2 -translate-y-1/2 transform rounded-full bg-gradient-to-br from-background to-transparent blur-3xl dark:from-foreground/5" />
    ...
    <div className="max-container z-10 flex flex-col justify-between">
      <div className="mb-8 flex flex-col-reverse items-start gap-6 pt-12 lg:flex-row 2xl:gap-10">
        ...
        <div className="flex w-full flex-col gap-8 md:basis-[40%]">
          <div className="w-full">
            <h1 className="mb-4 flex flex-col gap-3 text-4xl font-light md:text-5xl md:max-lg:flex-row lg:flex-col lg:gap-0 lg:text-6xl">
              <p>Subscribe to</p>
              <p>our newsletter</p>
            </h1>
            <FooterInput />
      ...
      <div className="flex items-center justify-center border-t border-t-border/70 py-4 text-center text-xs max-sm:flex-col max-sm:gap-2">
